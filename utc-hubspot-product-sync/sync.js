...
// - Bilingual FR/EN names via DeepL auto-translation
...
const DEEPL_API          = 'https://api-free.deepl.com/v2/translate'; // change to api.deepl.com if on paid plan
...
async function translateToEnglish(text) {
  ...
  try {
    ...
    if (!res.ok) {
      log('⚠️', `DeepL error ${res.status} — using French as fallback`);
      return text;
    }
    ...
    log('⚠️', `DeepL failed: ${err.message} — using French as fallback`);
    return text;
  }
}
...
async function main() {
  ...
  if (!DEEPL_KEY) missing.push('DEEPL_API_KEY');
  if (missing.length) {
  ...
  for (const product of products) {
    ...
    for (const variant of product.variants) {
      ...
      const frName = suffix ? `${frTitle} — ${suffix}` : frTitle;
      const enName = suffix ? `${enTitle} — ${suffixParts.join(' — ')}` : enTitle;
